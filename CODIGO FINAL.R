...
convert_columns_to_date <- function(df, columns) {
  for (col in columns) {
  df[[col]] <- dmy_hm(df[[col]])#Fechas a formato legible
  }
  return(df)}
...
convert_columns_to_numeric <- function(df, columns) {
  for (col in columns) {
    df[[col]] <- as.numeric(gsub(",", ".", df[[col]]))#Comas cambiamos por puntos
  }
  return(df)}
...
aaa %>%  mutate(imc=weight/((size/100)^2),
               ...
               nyha = ifelse(is.na(nyha),"nyha_1",nyha),  #NYHAS no apuntados son 1s?
...
#muertos con bomba 13
aaa %>% filter(!is.na(date_death)) %>% filter(ecls=="ecls") %>%  select(nhc)
